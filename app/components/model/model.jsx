...
export const Model = ({
  ...
  useEffect(() => {
    ...
    renderer.current.setPixelRatio(Math.min(devicePixelRatio, 2));
    ...
    // The parent decides the camera distance. Keep the Three.js camera
    // stable so CSS/container resizing cannot unexpectedly zoom the model.
  ...
  // Keep the WebGL viewport tied to the actual model container.
  useEffect(() => {
    const handleResize = () => {
      ...
      // Keep camera distance controlled by the caller. Only the aspect
      // ratio changes when the WebGL container changes size.
      camera.current.position.set(
        cameraPosition.x,
        cameraPosition.y,
        cameraPosition.z
  ...
  }, [cameraPosition.x, cameraPosition.y, cameraPosition.z, renderFrame]);
